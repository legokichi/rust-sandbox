(get-info :version)
; (:version "4.8.6")
; Started: 2020-05-19 03:52:30
; Silicon.version: 1.1-SNAPSHOT (ebfc142c+@(detached))
...
(assert (forall ((x $PSF<$Snap>)) (!
    ...
    :qid |$Snap.$SnapTo$PSF<$Snap>To$Snap|
    )))
(assert (forall ((x $Snap)) (!
    (= x ($SortWrappers.$PSF<$Snap>To$Snap($SortWrappers.$SnapTo$PSF<$Snap> x)))
    :pattern (($SortWrappers.$SnapTo$PSF<$Snap> x))
    :qid |$Snap.$PSF<$Snap>To$SnapTo$PSF<$Snap>|
    )))
...
; ////////// Symbols
; /predicate_snap_sets_declarations.smt2 [Snap]
...
(assert (=
  ($Snap.second ($Snap.second ($Snap.second ($Snap.second $t@11@00))))
  ($Snap.combine
    ($Snap.first ($Snap.second ($Snap.second ($Snap.second ($Snap.second $t@11@00)))))
    ($Snap.second ($Snap.second ($Snap.second ($Snap.second ($Snap.second $t@11@00))))))))
(set-option :timeout 10)
(push) ; 2
(assert (not (=
  ($SortWrappers.$SnapTo$Ref ($Snap.first ($Snap.second ($Snap.second $t@11@00))))
  ($SortWrappers.$SnapTo$Ref ($Snap.first ($Snap.second ($Snap.second ($Snap.second ($Snap.second $t@11@00)))))))))
(check-sat)
; unknown
(pop) ; 2
; 0.00s
...
(push) ; 1
(declare-const $t@13@00 Int)
(assert (not (= self@12@00 $Ref.null)))
(pop) ; 1
; ---------- DeadBorrowToken$ ----------
(declare-const borrow@14@00 Int)
